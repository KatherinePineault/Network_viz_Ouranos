...
setwd(data_path)
nodes<- read.csv("liens_prog_maritime_NODES.csv", sep=";", header=T, as.is=T)
nodes<- nodes[complete.cases(nodes),] ## ajoute parce qu'il y avait des lignes avec des NA a la fin
links<- read.csv("liens_prog_maritime_EDGES.csv", sep=";", header=T, as.is=T)
...
V(network)$label[45]<-"Regroupement\nPêcheurs professionnels\n sud Gaspésie"
V(network)$label[40]<-"Atlantic\n  Climate Adaptation\n  Solutions Association"
V(network)$label[34]<-"Saint-André-de-Kamouraska"

setwd(data_path)
load("Maritime_coordonnees.Rda")
setwd(output_path)

#tk_plot<- tkplot(network)
#l<- tkplot.getcoords(tk_plot)

#save(l, file="Maritime_coordonnees.Rda")

png(file="Visualisation_MARITIME.png", width=2000, height=2000)
plot(network, 
