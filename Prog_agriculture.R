...
setwd(data_path)
nodes<- read.csv("liens_prog_agriculture_NODES.csv", sep=";", header=T, as.is=T)
nodes<- nodes[complete.cases(nodes),] ## ajoute parce qu'il y avait des lignes avec des NA a la fin
links<- read.csv("liens_prog_agriculture_EDGES.csv", sep=";", header=T, as.is=T)
...
#save(l, file="Agriculture_coordonnees.Rda")

png(file="Visualisation_AGRICULTURE.png", width=2000, height=2000)
plot(network, 
