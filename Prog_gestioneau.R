...
setwd(data_path)
nodes<- read.csv("liens_prog_gestioneau_NODES.csv", sep=";", header=T, as.is=T)
nodes<- nodes[complete.cases(nodes),] ## ajoute parce qu'il y avait des lignes avec des NA a la fin
links<- read.csv("liens_prog_gestioneau_EDGES.csv", sep=";", header=T, as.is=T)
...
#setwd(data_path)
load("GestionEau_coordonnees.Rda")
setwd(output_path)

#tk_plot<- tkplot(network)
#l<- tkplot.getcoords(tk_plot)

#save(l, file="GestionEau_coordonnees.Rda")

png(file="Visualisation_GESTIONEAU.png", width=2000, height=2000)
plot(network, 
