...
setwd(data_path)
nodes<- read.csv("liens_prog_bati_NODES.csv", sep=";", header=T, as.is=T)
nodes<- nodes[complete.cases(nodes),] ## ajoute parce qu'il y avait des lignes avec des NA a la fin
links<- read.csv("liens_prog_bati_EDGES.csv", sep=";", header=T, as.is=T)
...
setwd(data_path)
load("Bati_coordonnees.Rda")
setwd(output_path)

#tk_plot<- tkplot(network)
#l<- tkplot.getcoords(tk_plot)

#save(l, file="Bati_coordonnees.Rda")

png(file="Visualisation_BATI.png", width=2000, height=2000)
plot(network, 
