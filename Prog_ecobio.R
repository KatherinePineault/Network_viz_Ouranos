...
setwd(data_path)
nodes<- read.csv("liens_prog_ecobio_NODES.csv", sep=";", header=T, as.is=T)
nodes<- nodes[complete.cases(nodes),] ## ajoute parce qu'il y avait des lignes avec des NA a la fin
links<- read.csv("liens_prog_ecobio_EDGES.csv", sep=";", header=T, as.is=T)
...
V(network)$size<-11+ (V(network)$budget.projet/200000)
...
##V(network_nrj)$label[10]<- "Base de données\ninteractive"
V(network)$label[39]<-"Newfoundland \nand Labrador Gov"
V(network)$label[66]<-"Nature\n Conservancy \nCan"
V(network)$label[4]<- "Aména\nAgro-Faunique"
V(network)$label[6]<- "Corridors\nécologiques"
V(network)$label[68]<-"Corridor\nAppalachien"
V(network)$label[56]<- "Groupe\nRousseau\nLefebvre"

setwd(data_path)
#load("EcoBio_coordonnees.Rda")
setwd(output_path)

#tk_plot<- tkplot(network)
#l<- tkplot.getcoords(tk_plot)

#save(l, file="EcoBio_coordonnees.Rda")

png(file="Visualisation_ECOBIO.png", width=2000, height=2000)
plot(network, 
