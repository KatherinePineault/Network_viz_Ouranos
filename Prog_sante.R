...
setwd(data_path)
nodes<- read.csv("liens_prog_sante_NODES.csv", sep=";", header=T, as.is=T)
nodes<- nodes[complete.cases(nodes),] ## ajoute parce qu'il y avait des lignes avec des NA a la fin
links<- read.csv("liens_prog_sante_EDGES.csv", sep=";", header=T, as.is=T)
...
V(network)$label[35]<- "Office municipal\n d'habitation \n Longueuil"


setwd(data_path)
load("Sante_coordonnees.Rda")
setwd(output_path)

#tk_plot<- tkplot(network)
#l<- tkplot.getcoords(tk_plot)

#save(l, file="Sante_coordonnees.Rda")

png(file="Visualisation_SANTE.png", width=2000, height=2000)
plot(network, 
