...
setwd(data_path)
nodes<- read.csv("liens_prog_foret_NODES.csv", sep=";", header=T, as.is=T)
nodes<- nodes[complete.cases(nodes),] ## ajoute parce qu'il y avait des lignes avec des NA a la fin
links<- read.csv("liens_prog_foret_EDGES.csv", sep=";", header=T, as.is=T)
...
V(network)$label[6]<- "Regénération\nfeux"
V(network)$label[34]<-"Tree and Timber\n Institute\n Florence"
V(network)$label[34]<-"Tree and Timber\n Institute\n Florence"
V(network)$label[36]<-"Consortium\n Forêt Boréale\n Commerciale"
V(network)$label[39]<-"GRIFF"
V(network)$label[60]<-"Institut recherche \nbiologie végétale"
V(network)$label[48]<-"Chaire\n CRSNG-UQAT-UQAM"
V(network)$label[38]<-"Coop R&D forestier\n AT NordQc"

setwd(data_path)
load("Foret_coordonnees.Rda")
setwd(output_path)

#tk_plot<- tkplot(network)
#l<- tkplot.getcoords(tk_plot)

#save(l, file="Foret_coordonnees.Rda")

png(file="Visualisation_FORET.png", width=2000, height=2000)
plot(network, 
