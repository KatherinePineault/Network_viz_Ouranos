...
setwd(data_path)
nodes<- read.csv("liens_SAC_NODES_avecEVAP.csv", sep=";", header=T, as.is=T)
#nodes<- nodes[complete.cases(nodes),] ## ajoute parce qu'il y avait des lignes avec des NA a la fin
links<- read.csv("liens_SAC_EDGES_avecEVAP.csv", sep=";", header=T, as.is=T)
...
#V(network)$label[10]<- "Base de données\ninteractive"
## Voir si c'est possible de ne pas le faire à la main
#V(network)$label[44]<-"Nansen Environmental\n and Remote Sensing \nCentre"
#V(network)$label[6]<-"Alternative dataset\n hydro science"
...
#setwd(data_path)
load("Simulations_coordonnees_2.Rda")


#tk_plot<- tkplot(network)
#l<- tkplot.getcoords(tk_plot)

#save(l, file="Simulations_coordonnees_2.Rda")

png(file="Visualisation_SIMULATIONS_2.png", width=2000, height=2000)
plot(network, 
